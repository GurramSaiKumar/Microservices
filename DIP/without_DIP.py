class DataAccessService:
    def fetch_data(self):
        return 50


class BusinessLogic:
    def __init__(self, data_service: DataAccessService):
        self.data_service = data_service

    def perform_business_logic(self):
        data = self.data_service.fetch_data()
        if data is not None:
            print("Received Data")
        else:
            print("No data received")

        print(f"Data after manipulation {data + 10}")


# client code.
# So if anything changes in DataAccessService, we need to change in BusinessLogic class.
# This is because BusinessLogic is depending on concrete classes not on abstract classes.
if __name__ == '__main__':
    data_access_service = DataAccessService()
    business_logic = BusinessLogic(data_access_service)
    business_logic.perform_business_logic()
